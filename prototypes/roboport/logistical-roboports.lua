...
function LogisticalRoboport.new(levels)
    ...
    self.logistics_radius = settings.logistical_logistics_radius:get()
        + (settings.logistical_logistics_radius_modifier:get() * self.levels.logistic_area)
    self.construction_radius = settings.logistical_construction_radius:get()
        + (settings.logistical_construction_radius_modifier:get() * self.levels.construction_area)
    self.robot_slots_count = settings.logistical_robot_slots:get()
        + (settings.logistical_robot_modifier:get() * self.levels.robot_storage)
...
function LogisticalRoboport:_apply_energy()
    ...
    self.charging_energy = tostring(charging_energy:with_scale(#self.charging_offsets or self.charging_station_count))
...
function LogisticalRoboport:get_suffix()
    return tostring(
        "c"
            .. number.within_bounds(self.levels.construction_area, 0, settings.construction_area_limit:get())
            .. "l"
            .. number.within_bounds(self.levels.logistic_area, 0, settings.logistic_area_limit:get())
            .. "r"
            .. number.within_bounds(self.levels.robot_storage, 0, settings.robot_storage_limit:get())
            .. "m"
            .. number.within_bounds(self.levels.material_storage, 0, settings.material_storage_limit:get())
    )
end
